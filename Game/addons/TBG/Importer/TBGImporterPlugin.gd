...
func _get_import_options(path, preset):
	return [
		...
		{
			"name": "Optimization/PruneResetTracks",
			"description": "Remove tracks that are found to provide the same value that the RESET track does already, requires that you play the RESET track before switching animations, or use an Animation Tree for driving your Animation.",
			"default_value": false
		}
	]
...
class TextureImportResult:
	...
	func _init(texture, width, height):
...
func _import_texture(entry: PackedByteArray, compress: bool) -> TextureImportResult:
	...
	return TextureImportResult.new(texture, image.get_width(), image.get_height())
...
class HarmonySpriteInfo:
	...
	func _init(texture, offset, scale):
		self.texture = texture
		self.offset = offset
		self.scale = scale

func SpriteSheetToSpriteInfos(data, global_path, compress):
	var textures = []
	var result = {}
...
func _import(source_file, save_path, options, platform_variants, gen_files):
	...
	# Load textures.
	var sstsi = SpriteSheetToSpriteInfos(xml_data, global_path, true)
	var sprite_sheet_to_sprite_infos = sstsi.result
	
	var texture_assets =  sstsi.textures
	if sprite_sheet_to_sprite_infos.size() < 1:
	...
	for stage in stages:
		...
		var name_ends_with_loop = stage.name.to_lower().ends_with("-loop") or stage.name.to_lower().ends_with("-cycle")
		...
		for node_entry in created_hierarchy_nodes:
			...
			if not node_is_deform_bone:
				...
				build_animation_tracks.call(animation, framerate, node, Animation.UPDATE_CONTINUOUS, func(frame):
					return {
		...
		for sprite_entry in created_drawing_nodes:
			...
			build_animation_tracks.call(animation, framerate, sprite_node, Animation.UPDATE_CONTINUOUS, func(frame):
				var z_sum = 0.0
		...
		if options["Optimization/CompressAnimationTracks"]:
			for node_entry in created_hierarchy_nodes:
				...
				build_animation_track.call(animation, framerate, node, "transform", Animation.UPDATE_CONTINUOUS, func(frame):
	...
	# Go back through all animations and delete any tracks that have a single keyframe that is already the RESET keyframe
	if options["Optimization/PruneResetTracks"]:
		for stage in stages:
			var animation = animation_library.get_animation(stage.name)
			var _track_count = animation.get_track_count()
			for track_id in range(_track_count - 1, -1, -1):
				var key_count = animation.track_get_key_count(track_id)
				if key_count == 1:
					var key_value = animation.track_get_key_value(track_id, 0)
					var track_path = animation.track_get_path(track_id)
					var reset_track = reset_animation.find_track(track_path, Animation.TYPE_VALUE)
					if reset_track == -1:
						continue
					var reset_key_value = reset_animation.track_get_key_value(reset_track, 0)
					if track_path.to_string().ends_with("transform"): # HACK - transform tracks are special
						var transform = key_value
						var reset_transform = reset_key_value
						if transform == reset_transform: # Equality needs to be specially checked for these transforms, otherwise they are never equal. (TODO - Fix in the engine?)
							animation.remove_track(track_id)
					elif key_value.is_equal_approx(reset_key_value):
						animation.remove_track(track_id)
...
func build_animation_tracks(animation, framerate, node, update_mode, frame_to_prop_to_value):
	...
	for frame in range(int(animation.length * float(framerate))):
		...
		for key in prop_to_value:
			...
			if previous_frame_value == null or not previous_frame_value == prop_to_value[key]:
