...
# { "my/short/path": { tbg: "res://tbg/my/short/path.stg", source: "/Users/Docs/Project/Assets/my/short/path.stage" }
...
func update_ui():
	...
	var tbg_assets = get_file_tree("res://tbg/", "tbg")
	...
	asset_tree.content = asset_keys.map(func(key): 
		...
		if value.get("tbg"):
			if (value["tbg"] as String).ends_with("\t"):
				...
				result["tooltip"] = "Source asset for: %s" % trim_path(value["tbg"], "res://tbg/")
...
func _on_tree_context_menu(path, screen_position):
	...
	if tbg_path:
		var tbgfilename = trim_path(tbg_path, "res://tbg/")
	...
	if asset_source:
		...
		if tbg_path and path != tgsfilename and trim_path(tbg_path, "res://tbg/") != tgsfilename:
